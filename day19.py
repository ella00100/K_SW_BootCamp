...
def factorial_input():
    lbl_results.config(text=f"팩토리얼 값 : {fact_recu(int(en_num_input.get()))}")


def fibonacci_input():
    lbl_results.config(text=f"피보나치 수 : {fibo_memo_recu(int(en_num_input.get()))}")


win = tk.Tk()  # 윈도우 생성
win.title("Calculator")  # 피보나치, 팩토리얼 계산기
win.geometry("300x150")  # 가로, 세로 너비 조정
