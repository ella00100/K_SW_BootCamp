...
def printGraph(g) :
    print('   ', end = ' ')
    for first_row in range(g.SIZE) :   # 첫 행에 정점 출력
        print(name_array[first_row], end =' ')
    print()
    for row in range(g.SIZE) : # 한 행씩 돌면서
        print(name_array[row], end =' ')
        for col in range(g.SIZE) : # 각 열에 엣지 출력
